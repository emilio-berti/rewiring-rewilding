...
#write_rds("../../Results/Bayesian_TL.rds")
...
dat %>% 
  ...
  ggplot() +
  geom_boxplot(aes(Scenario, Proportion, fill = Size), show.legend = F, outlier.shape = NA) +
  geom_jitter(aes(Scenario, Proportion, shape = Ecozone), alpha = 0.1, show.legend = T, width = 0.3) +
  facet_wrap(Size ~., ncol = 2, scales = "free", labeller = size_labeller2) +
  scale_fill_manual(values = wesanderson::wes_palette("Zissou1", 5)) +
  scale_shape_manual(values = c(1, 3, 5, 7, 8, 9),
                     labels = c("Australasia", "Afrotropic", "Indomalaya", "Neartic", "Neotropic", "Paleartic")) +
  monkey_theme + 
  xlab("Scenario") + 
  ylab("Number of species") +
