<<<<<<< HEAD
infer_current <- function(protected_area, current_stack){
  # protected_area <- st_simplify(protected_area, dTolerance = 1000) #does not reduce a lot computation time
  # registerDoParallel(50)
  foreach(x = names(current_stack), .packages = "raster") %dopar% {
    intersect(current_stack[[x]], protected_area) # intersect and crop take the same time; intersect fits more our purpose
  } %>% 
